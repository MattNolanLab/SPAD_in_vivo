...
%        pause(1);
      ...
      colormap('default');
        ...
        if b_saved==1,
            ...
            imageCaptured=rot90(reshape(max(sum_frame,0),320,240),1);
        end;
        %std(imagestream)
...
for ti=1:blocks,
    ...
    % TOGGLE BUTTON 3, trig==0 means not using triggered mode.
    if trig == 0,
      disp('trigger is off');
      trigger(evalin('base','s.okComms'), evalin('base','s.bank'),'ADC_FIFO_RST');
      trigger(evalin('base','s.okComms'), evalin('base','s.bank'),'EXPOSURE_START_TRIGGER');
    ...
    if trig == 1,
        ...
        disp('First update');
        disp(~istriggered(evalin('base','s.okComms'), 96, 1));
        updatetriggerouts(evalin('base','s.okComms'));
        disp('Second update');
        disp(~istriggered(evalin('base','s.okComms'), 96, 1));       
    ...
    % SHOULD BE UNCOMMENT FOR TRIGGERED MODE??----Yifang
    %trigger(evalin('base','s.okComms'), evalin('base','s.bank'),'PROG_CTRL_SR');
    %trigger(evalin('base','s.okComms'), evalin('base','s.bank'),'EXPOSURE_START_TRIGGER');        
    end;
