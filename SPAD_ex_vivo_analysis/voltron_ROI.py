...
def butter_filter(data, btype='high', cutoff=10, fs=10000, order=5):
...
def get_spikes(trace, superfactor=10, threshs=(.4, .6, .75)):
    ...
    for iters in range(3):
        ...
        high_freq_med = np.array(pd.Series(high_freq).rolling(window=250,min_periods=250,center=True).median())
        high_freq_std = np.array(pd.Series(high_freq).rolling(window=250,min_periods=250,center=True).std())       
        trace_med = np.array(pd.Series(sub_thresh1).rolling(window=250,min_periods=250,center=True).median())
        trace_std = np.array(pd.Series(sub_thresh1).rolling(window=250,min_periods=250,center=True).std())
